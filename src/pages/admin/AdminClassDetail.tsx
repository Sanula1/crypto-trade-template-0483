...
export default function AdminClassDetail() {
  ...
  const inp = "w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500/30";
  ...
  return (
    <div className="space-y-5">
          ...
          <div className="p-6 flex-1">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                {cls.subject && <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{cls.subject}</p>}
            ...
            <div className="flex flex-wrap gap-5 mt-4 text-xs text-slate-500 dark:text-slate-400">
              {cls.monthlyFee != null && <span className="font-bold text-blue-600 dark:text-blue-400 text-sm">Rs. {Number(cls.monthlyFee).toLocaleString()} / month</span>}
              <span className="flex items-center gap-1"><span className="font-semibold text-slate-700 dark:text-slate-200">{months.length}</span> month{months.length !== 1 ? 's' : ''}</span>
              <span className="flex items-center gap-1"><span className="font-semibold text-slate-700 dark:text-slate-200">{recordings.length}</span> recording{recordings.length !== 1 ? 's' : ''}</span>
              <span className="flex items-center gap-1"><span className="font-semibold text-slate-700 dark:text-slate-200">{enrollments.length}</span> student{enrollments.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-slate-100 dark:bg-slate-800/80 rounded-xl p-1 border border-slate-200 dark:border-slate-700 overflow-x-auto">
        {([['months', 'Months'], ['recordings', 'Recordings'], ['students', 'Students'], ['attendance', 'Attendance']] as [Tab, string][]).map(([key, lbl]) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex-1 min-w-[4.5rem] px-3 sm:px-4 py-2 rounded-lg text-xs font-semibold transition whitespace-nowrap ${tab === key ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-100 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}>
            {lbl}
            ...
            <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={() => setShowMonthForm(false)}>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700 rounded-t-2xl">
                  ...
                  <button onClick={() => setShowMonthForm(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                <form onSubmit={saveMonth} className="p-5 space-y-3">
                  {monthError && <div className="flex items-center gap-2 p-3.5 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 text-sm text-red-600 dark:text-red-400"><svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{monthError}</div>}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => setShowMonthForm(false)} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition">Cancel</button>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
            {months.length === 0 ? (
              <div className="p-10 text-center">
                ...
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No months yet</p>
        ...
        <div className="space-y-3">
            ...
            <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={() => setShowRecForm(false)}>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-md" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700 rounded-t-2xl">
                  ...
                  <button onClick={() => setShowRecForm(false)} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                <form onSubmit={saveRec} className="p-5 space-y-3">
                  {recError && <div className="flex items-center gap-2 p-3.5 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 text-sm text-red-600 dark:text-red-400"><svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>{recError}</div>}
                  ...
                  <div className="flex gap-3 pt-2">
                    <button type="button" onClick={() => setShowRecForm(false)} className="flex-1 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition">Cancel</button>
          ...
          {filteredRecs.length === 0 ? (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-10 text-center">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No recordings {filterMonth ? 'in this month' : 'yet'}</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Add your first recording to get started</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredRecs.map((rec: any) => (
                <div key={rec.id} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden group hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-md transition-all">
                  {/* Thumbnail */}
                  ...
                  <div className="p-3.5">
                    ...
                    <div className="flex items-center gap-1.5 mt-3 pt-2.5 border-t border-slate-100 dark:border-slate-700">
                      ...
                      {rec.videoUrl && (
                        <a href={rec.videoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-xs font-semibold hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition">
                      ...
                      <button onClick={() => deleteRec(rec.id)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 text-xs font-semibold hover:bg-red-100 dark:hover:bg-red-900/40 transition ml-auto">
                        <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        Delete
                      </button>
                    </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
            {enrollments.length === 0 ? (
              <div className="p-10 text-center">
                ...
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No students enrolled yet</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
          {watchSessions.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No watch sessions yet</p>
