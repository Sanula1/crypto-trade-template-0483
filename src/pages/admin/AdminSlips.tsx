...
export default function AdminSlips() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div>
        <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">Payment Slips</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Review and verify student payment submissions</p>
      </div>

      {/* Stat chips */}
      <div className="flex flex-wrap gap-3">
        {[{label: 'Pending', count: counts.PENDING, color: 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800'},
          {label: 'Verified', count: counts.VERIFIED, color: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800'},
          {label: 'Rejected', count: counts.REJECTED, color: 'bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800'},
      ...
      {preview && createPortal(
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setPreview(null)}>
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50"><img src={preview.slipUrl} alt="slip" className="w-full object-contain max-h-80 rounded-xl" /></div>
