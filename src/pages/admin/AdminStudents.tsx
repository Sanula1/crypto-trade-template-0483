...
export default function AdminStudents() {
  ...
  const studentColumns: readonly StickyColumn<any>[] = [
    { id: 'instituteId', label: 'Student ID', minWidth: 130, render: (s) => <span className="font-mono text-xs text-blue-600 dark:text-blue-400 font-bold">{s.profile?.instituteId || '-'}</span> },
    {
      id: 'name',
      label: 'Name',
      minWidth: 200,
      render: (s) => (
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-slate-800 dark:text-slate-100 text-sm">{s.profile?.fullName || '-'}</span>
        </div>
      ),
    },
    { id: 'email', label: 'Email', minWidth: 220, render: (s) => <span className="text-slate-500 dark:text-slate-400 text-sm">{s.email}</span> },
    { id: 'phone', label: 'Phone', minWidth: 140, render: (s) => <span className="text-slate-500 dark:text-slate-400 text-sm">{s.profile?.phone || '-'}</span> },
    { id: 'school', label: 'School', minWidth: 160, render: (s) => <span className="text-slate-500 dark:text-slate-400 text-sm">{s.profile?.school || '-'}</span> },
    {
      id: 'status',
      label: 'Status',
      minWidth: 120,
    ...
    { id: 'joined', label: 'Joined', minWidth: 110, render: (s) => <span className="text-slate-400 dark:text-slate-500 text-xs">{s.createdAt ? new Date(s.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</span> },
    {
      id: 'actions',
      label: 'Actions',
      minWidth: 180,
      ...
      render: (s) => (
        <div className="flex items-center justify-end gap-1.5">
          <button onClick={() => openEdit(s)} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 text-xs font-semibold hover:bg-blue-100 dark:hover:bg-blue-900/40 transition">
          ...
          <button onClick={() => handleDelete(s.id)} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500 dark:text-red-400 text-xs font-semibold hover:bg-red-100 dark:hover:bg-red-900/40 transition">
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-slate-100">Students</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">{students.length} registered students</p>
        </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, email, ID..."
                className="pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-sm bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-400 shadow-sm transition w-full sm:w-64" />
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={() => setShowForm(false)}>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700 rounded-t-2xl">
              <div>
                <h2 className="font-bold text-slate-800 dark:text-slate-100">{editingStudent ? 'Edit Student' : 'New Student'}</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="col-span-1 sm:col-span-2">
                  ...
                  <input type="text" value={form.fullName} onChange={e => setForm(p => ({ ...p, fullName: e.target.value }))} placeholder="John Doe" required
                    className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
                </div>
                {!editingStudent && (
                  <>
                    <div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-6 space-y-3">{[1,2,3,4].map(i => <div key={i} className="h-14 rounded-xl bg-slate-100 dark:bg-slate-700 animate-pulse" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{search ? 'No students match your search' : 'No students registered yet'}</p>
          </div>
        ) : (
          <StickyDataTable
