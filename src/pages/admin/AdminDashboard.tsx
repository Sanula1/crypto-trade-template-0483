    ...
    <div className="w-full space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {STAT_CARDS.map(({ key, label, gradient, shadow, icon }) => (
          <div key={key} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-shadow">
            ...
            {loading ? (
              <div className="h-8 w-16 bg-slate-100 dark:bg-slate-700 rounded-lg mt-1.5 animate-pulse" />
            ) : (
              <p className="text-3xl font-bold text-slate-800 dark:text-slate-100 mt-1">{counts[key] ?? 0}</p>
            )}
          </div>
        ))}
      </div>
