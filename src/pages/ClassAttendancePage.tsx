...
const statusConfig: Record<string, { label: string; dot: string; badge: string }> = {
  ...
  INCOMPLETE: { label: 'Incomplete', dot: 'bg-amber-400',   badge: 'bg-amber-50 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' },
  MANUAL:     { label: 'Manual',     dot: 'bg-blue-500',    badge: 'bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' },
};
...
export default function ClassAttendancePage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      {/* Back */}
      <Link to={`/classes/${id}`} className="inline-flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-100 transition font-medium">
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-bold text-slate-800 dark:text-slate-100">Attendance</h1>
          ...
          {(['ALL', 'COMPLETED', 'INCOMPLETE', 'MANUAL'] as const).map(s => {
            ...
            return (
              ...
              >
                ...
                <span className={`ml-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-bold ${active ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-600 text-slate-600 dark:text-slate-300'}`}>
                  {counts[s]}
                </span>
              </button>
            );
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">No attendance records found</p>
