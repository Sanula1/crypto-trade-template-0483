...
export default function WatchHistoryPage() {
  ...
  return (
    <div className="w-full space-y-6 animate-fade-in">
      ...
      {sessions.length === 0 ? (
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 p-14 text-center shadow-sm">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">No watch history yet</p>
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Start watching recordings to see your history here</p>
        ...
        Object.entries(grouped).map(([dateKey, daySessions]) => (
          <div key={dateKey} className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 overflow-hidden shadow-sm">
            {/* Date header */}
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-700">
              {daySessions.map((s: any) => (
                <div key={s.id} className="p-4 sm:p-5 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                  <div className="flex items-start gap-4">
                    {/* Thumbnail */}
                    <div className="w-20 h-14 rounded-lg overflow-hidden bg-slate-200 dark:bg-slate-600 flex-shrink-0">
                    ...
                    <div className="flex-shrink-0">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold ${
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${
                          ...
                          s.status === 'PAUSED' ? 'bg-yellow-500' : 'bg-slate-400'
                        }`} />
