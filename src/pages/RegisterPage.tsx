...
export default function RegisterPage() {
  ...
  const inputCls = "w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-400 transition-all text-sm shadow-sm";

  const Field = ({ label, type = 'text', value, onChange, placeholder, required = false, hint }: any) => (
    <div>
      <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{label}{!required && <span className="text-slate-400 dark:text-slate-500 ml-1 text-xs font-normal">(optional)</span>}</label>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#0b1120] flex items-center justify-center p-6 relative transition-colors duration-300">
      {/* Theme toggle */}
      ...
      <div className="w-full max-w-[480px] animate-fade-in">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1.5 text-sm mb-8">Join ThilinaDhananjaya LMS to start learning</p>
            ...
            <div key={s} className="flex items-center gap-1 flex-1">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
                  ...
                  : i === step ? 'bg-gradient-to-br from-blue-500 to-blue-600 text-white shadow-lg shadow-blue-500/30 ring-4 ring-blue-100 dark:ring-blue-500/20'
                ...
                <span className={`text-xs font-semibold hidden sm:block ${i === step ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500'}`}>{s}</span>
              </div>
              {i < steps.length - 1 && <div className={`flex-1 h-[2px] rounded ml-1 transition-all ${i < step ? 'bg-blue-500' : 'bg-slate-200 dark:bg-slate-700'}`} />}
            </div>
          ))}
        </div>

        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 border border-slate-100 dark:border-slate-700 p-8 transition-colors duration-300">
          {error && (
          ...
          {step === 1 && (
            <form onSubmit={nextStep} className="space-y-5">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setStep(0)} className="flex-1 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition">Back</button>
            ...
            <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
              <div className="space-y-1">
                {[['Email', form.email], ['Name', form.fullName], ...(form.phone ? [['Phone', form.phone]] : []), ...(form.school ? [['School', form.school]] : [])].map(([k, v]) => (
                  <div key={k} className="flex justify-between items-center py-3 border-b border-slate-100 dark:border-slate-700 last:border-0">
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setStep(1)} className="flex-1 py-3 rounded-xl border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:bg-slate-50 dark:hover:bg-slate-700 transition">Back</button>
        ...
        <p className="mt-8 text-center text-sm text-slate-500 dark:text-slate-400">
          ...
          <Link to="/login" className="text-blue-600 dark:text-blue-400 font-semibold hover:text-blue-700 dark:hover:text-blue-300 transition">Sign in</Link>
        </p>
      </div>
    </div>
  );
